...
from app.tools.monitor_tools import register_monitor_tools
...
def create_app() -> FastAPI:
    ...
    monitor_provider = MonitorProvider(cfg.monitor) if cfg.monitor and cfg.monitor.enabled else None

    mcp = FastMCP("automation-mcp-server")
    register_homey_tools(mcp, homey_provider, logger)
    register_ha_tools(mcp, ha_provider, logger)
    register_monitor_tools(mcp, monitor_provider, logger)
    ...
    async def meta(_: Any = Depends(auth_dependency)) -> dict[str, Any]:
        return {
            ...
            "monitor_enabled": bool(monitor_provider),
